...
export class SearchService {
    ...
    async search(user: User, { keyword, user_id, index, count }: SearchDto) {
        ...
        const query = this.postRepo
            ...
            .andWhere("to_tsvector('vn', post.description) @@ websearch_to_tsquery('vn', :keyword)", {
                keyword,
            })
    ...
    async searchUser(user: User, { keyword, index, count }: SearchUserDto) {
        ...
        const query = this.userRepo
            ...
            .andWhere('blocking.id IS NULL')
            .where(
                "(to_tsvector('vn', user.username) @@ websearch_to_tsquery('vn', :keyword) OR user.username ILIKE :likeKeyword)",
                {
                    keyword,
                    likeKeyword,
                },
            )
            .orderBy({ 'user.lastActive': 'DESC', 'user.id': 'DESC' })
            .skip(index)
            .take(count);
